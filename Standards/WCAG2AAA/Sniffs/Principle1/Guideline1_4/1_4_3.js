        ...
        if (element === top) {
            ...
            for (var i = 0; i < failures.length; i++) {
                ...
                if (isAbsolute === true) {
                    ...
                    code += '.BgImage';
                    HTMLCS.addMessage(HTMLCS.WARNING, element, 'このテキストは背景画像の上に置かれています。背景とテキストのコントラスト比が少なくとも ' + required + ':1 以上あることを確認してください。 This element\'s text is placed on a background image. Ensure the contrast ratio between the text and all covered parts of the image are at least ' + required + ':1.', code);
                } else {
                    code += '.Fail';
